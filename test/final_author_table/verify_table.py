...
class FieldVerifier:
        ...
        try:
            ...
            if not query_template:
                return FieldTestResult(
                    ...
                    actual_value=None,
                    passed=False,
                    error_message="Query template not found"
                )

            # Get expected value from field calculation (now returns 0 on failure)
            expected_value = self.db_operations.execute_field_query(field_name, author_name, query_template)

            # Get actual value from final_author_table
            actual_value = self.db_operations.get_actual_value(field_name, author_name)
    ...
    def get_sample_authors(self, limit: int = 5) -> List[str]:
        ...
        """
        return self.db_operations.get_sample_authors(limit)
